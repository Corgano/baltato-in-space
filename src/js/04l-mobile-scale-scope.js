/*
 * Keep the mobile scaling correction from changing the desktop layout.
 */

function balttatoScaleGameFrameScoped() {
  const app = document.getElementById("app-container");
  if (!app) return;

  if (!balttatoIsMobileDevice()) {
...
window.balttatoScaleGameFrame = balttatoScaleGameFrameScoped;
balttatoScaleGameFrameScoped();
window.addEventListener("resize", balttatoScaleGameFrameScoped);
window.addEventListener("orientationchange", () => {
  setTimeout(balttatoScaleGameFrameScoped, 100);
});
if (window.visualViewport) {
  window.visualViewport.addEventListener("resize", balttatoScaleGameFrameScoped);
}
